...
def read_cal(i2c,addr):
    ...
    return {'T1':dig_T1,'T2':dig_T2,'T3':dig_T3,'P1':dig_P1,'P2':dig_P2,'P3':dig_P3,'P4':dig_P4,'P5':dig_P5,'P6':dig_P6,'P7':dig_P7,'P8':dig_P8,'P9':dig_P9}
...
def compensate(cal,raw_t,raw_p):
    ...
    var2=var1*var1*cal['P6']/32768.0 if 'P6' in cal else 0
...
def aht_soft_reset(i2c,addr):
    try:
        i2c.writeto(addr,bytes([0xBA]))
        time.sleep_ms(20)
    except Exception:
        pass

def aht_init(i2c,addr):
    try:
        i2c.writeto(addr,bytes([0xBE,0x08,0x00]))
        time.sleep_ms(20)
    except Exception:
        pass

def aht_read(i2c,addr,timeout_ms=200):
    ...
    msb_h=(data[3] & 0x0F)
    raw_h=(msb_h<<16)|(data[4]<<8)|data[5]
    humidity=raw_h*100.0/(1<<20)
    msb_t=(data[0]<<12)|(data[1]<<4)|(data[2]>>4)
    temperature=msb_t*200.0/(1<<20)-50.0
...
def main():
    ...
    if not bmp_addr:
        ...
        return
    is_bme=(bmp_cid==0x60)
    print("Found device at 0x%02X chip id 0x%02X %s" % (bmp_addr,bmp_cid,"BME280" if is_bme else "BMP280"))
    cal=read_cal(i2c,bmp_addr)
    write_config(i2c,bmp_addr)
    aht_soft_reset(i2c,aht_addr)
    aht_init(i2c,aht_addr)
    i=0
    while True:
        try:
            raw_t,raw_p=read_raw_bmp(i2c,bmp_addr)
            temp,press=compensate(cal,raw_t,raw_p)
            aht_t,aht_h=aht_read(i2c,aht_addr)
            hum_str="N/A"
            if aht_h is not None:
                hum_str="%.2f %% (AHT20)" % aht_h
            print("Temperature: %.2f C Pressure: %.2f hPa Humidity: %s, %d" % (temp, press/100.0, hum_str, i))
            i=i+1
